...
Single-file self-contained frontend at `api/static/index.html`. No separate JS/CSS files.

### Design
- Dark theme with green accent (`#00FF88`)
- Fonts: DM Sans (UI) + Space Mono (data)
- Leaflet map with CartoDB Dark Matter tiles (no API key needed)
...
### Architecture
- 4-step wizard: Location → Provider/Model → Task/Tokens → Results
- Token-based calculator with 10 real-world scenarios + custom prompt analyzer
- Cross-model comparison table (all models ranked by CO₂)
- Scale calculator (daily/monthly/yearly projections)
- Region switch advisor (shows cleanest alternative)

### API Integration
- On load, fetches `/v1/regions` to overlay live data onto hardcoded `DATACENTERS`
- Falls back gracefully to hardcoded data if API unavailable
- Data source indicator in header shows "Live API" (green) or "Cached" (dim)
- Frontend `DATACENTERS[].id` format matches API's `{provider}-{region_code}`
